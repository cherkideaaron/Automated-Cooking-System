...
export default function CustomScreen() {
  ...
  const generateFullRecipe = async () => {
    if (!foodName.trim()) {
      Toast.show({
        type: 'error',
        text1: 'Food Name Required',
        text2: 'Enter a dish name (e.g., "Omelette") first',
        position: 'bottom'
      });
      return;
    }
    ...
    if (!currentKey) {
      Toast.show({
        type: 'error',
        text1: 'API Key Required',
        text2: 'Configure Gemini API key in Settings',
        position: 'bottom'
      });
      return;
    }
    ...
    setIsGenerating(true);
    try {
      const result = await geminiService.generateFullRecipe(currentKey, foodName);
      ...
      Toast.show({
        type: 'success',
        text1: 'Recipe Magic!',
        text2: `Generated ${result.ingredients.length} ingredients and ${result.steps.length} steps`,
        position: 'bottom'
      });
    } catch (error) {
      ...
      Toast.show({
        type: 'error',
        text1: 'Magic Failed',
        text2: 'Try a different food name or check your key',
        position: 'bottom'
      });
    } finally {
      setIsGenerating(false);
  ...
  const selectSuggestion = (suggestion: RecipeSuggestion) => {
    ...
    setFoodName(suggestion.name); // Set the food name from the suggestion
  ...
  const publishRecipe = async () => {
    ...
    try {
      ...
      // Logic moved to recipeService.getUserProfile for robustness
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} showsVerticalScrollIndicator={false}>
      ...
      <Text style={[styles.label, { color: colors.textSecondary }]}>
        {mode === 'manual' ? 'Food Name' : 'Generated Food Name'}
      </Text>
      <TextInput
        value={foodName}
        onChangeText={setFoodName}
        placeholder={mode === 'manual' ? "Enter food name" : "Will be generated by AI"}
        placeholderTextColor={colors.textSecondary}
        style={[styles.input, { backgroundColor: colors.inputBackground, color: colors.text }]}
        editable={mode === 'manual' || !!selectedSuggestion}
      />

      {/* API KEY HELPER (AI MODE ONLY) */}
      {mode === 'ai' && !apiKey && (
        <Pressable
          style={[styles.apiKeyBanner, { backgroundColor: '#E5393522', borderColor: '#E53935' }]}
          onPress={() => router.push('/settings')}
        >
          <Ionicons name="warning-outline" size={18} color="#E53935" />
          <Text style={[styles.apiKeyBannerText, { color: colors.text }]}>
            Gemini API Key missing. <Text style={{ fontWeight: '800', textDecorationLine: 'underline' }}>Configure in Settings</Text>
          </Text>
        </Pressable>
      )}
      ...
      <View style={styles.sectionHeader}>
        <Text style={[styles.label, { color: colors.textSecondary }]}>
          {mode === 'ai' ? '1. List Ingredients You Like' : 'Ingredients'}
        </Text>
        <Pressable
      ...
      {mode === 'ai' && (
        <>
          <View style={styles.sectionHeader}>
            <Text style={[styles.label, { color: colors.textSecondary }]}>2. Generate Food Suggestions</Text>
            <Pressable
                ...
                {isExpanded && (
                  <View style={styles.dropdownContent}>
                    ...
                    >
                      ...
                      <Text style={styles.selectFoodText}>Choose This Food</Text>
                    </Pressable>
                  </View>
                )}
              </View>
